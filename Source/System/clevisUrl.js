 ...
 */
function process(string, modifiers)
{
    if ( ! string) throw new Error(1, '입력값이 비어 있습니다.');

    var p = new RegExp(patterns.url, modifiers);
...
function parse(url, type)
{
    ...
    // 쿼리스트링 분석
    if (r[9]) {
        r[9] = r[9].match(patterns.querystring);
...
module.exports = { process, collect }